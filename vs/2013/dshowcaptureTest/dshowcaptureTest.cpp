...
int main()
{
    ...
    for (auto i = 0; i < videoCount; i++) {
        printf("Name: %s Path: %s (%d)\r\n", videoDevices[i].name, videoDevices[i].path, videoDevices[i].capsCount);
    ...
    for (auto i = 0; i < audioCount; i++) {
        printf("Name: %s Path: %s (%d)\r\n", audioDevices[i].name, audioDevices[i].path, audioDevices[i].capsCount);
